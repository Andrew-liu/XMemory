...
function App() {
  ...
  async function go(next: Page) { try { if (generating.current) throw new Error('正在创作，请等待完成或先停止'); await providerSettings.current?.flush(); await editor.current?.flush(); setPage(next); setSelected(''); setCompose(false); setOutput(''); setRunStatus('') } catch(e) { setError((e as Error).message) } }
  async function open(id: string) { try { if (generating.current) throw new Error('正在创作，请等待完成或先停止'); await editor.current?.flush(); setSelected(id); setCompose(false); setOutput(''); setRunStatus('') } catch(e) { setError((e as Error).message) } }
  async function create() { try { if (generating.current) throw new Error('正在创作，请等待完成或先停止'); await providerSettings.current?.flush(); await editor.current?.flush(); const today = dateKey(); const n = await window.xm.save({ kind: 'inspiration', title: '未命名灵感', body: '', scheduledDate: today }); await refresh(); setCalendarDate(today); setPage('inspiration'); setSelected(n.id) } catch(e) { setError((e as Error).message) } }
  ...
  async function createTopic(title: string): Promise<boolean> {
    ...
    try {
      ...
      await refresh(); setPage('topic'); setSelected(created.id); setCompose(false); setRunStatus('')
      return true
